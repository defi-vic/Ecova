...
function Generator() {
  const { pickups, balance, transactions, impact, createPickup: createSharedPickup, classifyWaste, redeemReward, syncStatus, dataError, isMutating } = useEcovaData();
  ...
  const runAi = async () => { if (!wasteImage) { toast.error("Upload a waste photo before identifying material."); return; } const result = await classifyWaste(wasteImage, weight, selectedMaterials.join(" + ") || "Plastic"); setClassification(result); setAiDetected(true); };
  const createPickup = async () => {
    const id = await createSharedPickup({ material: selectedMaterials.join(" + ") || "Plastic", estimatedWeight: weight, location, pickupWindow: slot, imageData: wasteImage, detectedMaterial: classification?.detectedMaterial, aiConfidence: classification?.confidence, aiNotes: classification?.notes, contaminationEstimate: classification?.contaminationEstimate });
  ...
  return <div className="generator-app">
    ...
    <div className="generator-main">
      ...
      <div className="generator-content">
        ...
        {syncStatus === "degraded" && <div className="sync-warning" role="status"><ShieldCheck size={15} /><span><b>SYNC PAUSED</b> {dataError ?? "Latest backend data is unavailable."} Actions will not be marked complete until Ecova reconnects.</span></div>}
...
function FlowDrawer({ pickup, onClose }: { pickup: Pickup; onClose: () => void }) {
  ...
  const eventLabels = ["PICKUP_REQUESTED", "COLLECTOR_ASSIGNED", "WEIGHT_VERIFIED", "HUB_RECEIVED", "RECYCLING_CONFIRMED", "REWARD_ISSUED"];
...
function ScheduleFlow({ step, setStep, materials: selected, toggleMaterial, weight, updateWeight, location, setLocation, slot, setSlot, aiDetected, runAi, wasteImage, onImageSelected, classification, onClose, onConfirm, disabled }: {
  step: number;
  setStep: (step: number) => void;
  materials: Material[];
  toggleMaterial: (name: Material) => void;
  weight: number;
  ...
  classification: { detectedMaterial: string; confidence: number | null; contaminationEstimate: string; estimatedWeight: number; isFallback: boolean; notes: string } | null;
  onClose: () => void;
  onConfirm: () => void;
  disabled: boolean;
}) {
  const canContinue = step === 1 ? Boolean(wasteImage && selected.length && weight > 0) : step === 2 ? location.trim().length >= 3 : Boolean(wasteImage && selected.length && location.trim().length >= 3);
  return <div className="modal-backdrop"><section className="schedule-modal"><header className="schedule-header"><div><Kicker>SCHEDULE A PICKUP</Kicker><h2>{step === 1 ? "What are you recycling?" : step === 2 ? "Where should we collect?" : "Ready to move it?"}</h2></div><button onClick={onClose} aria-label="Close schedule dialog"><X size={19} /></button></header><div className="schedule-progress">{["MATERIAL", "LOCATION & TIME", "CONFIRM"].map((label, index) => <div className={step >= index + 1 ? "progress-active" : ""} key={label}><span>{String(index + 1).padStart(2, "0")}</span>{label}</div>)}</div>{step === 1 && <div className="schedule-body"><div className="material-select-grid">{materials.map(({ name, icon: Icon, color }) => <button className={selected.includes(name) ? "material-selected" : ""} key={name} onClick={() => toggleMaterial(name)} aria-pressed={selected.includes(name)}><span className={`material-select-icon guide-${color}`}><Icon size={18} /></span><b>{name}</b>{selected.includes(name) && <Check className="material-check" size={14} />}</button>)}</div><div className="weight-block"><div className="weight-label"><span>ESTIMATED WEIGHT</span><small>Final reward is based on verified weight.</small></div><div className="weight-control"><button onClick={() => updateWeight(-0.5)} aria-label="Decrease estimated weight"><Minus size={16} /></button><input type="number" value={weight} onChange={(event) => updateWeight(Number(event.target.value) - weight)} min="0.5" max="100" step="0.5" aria-label="Estimated weight in kilograms" /><span>KG</span><button onClick={() => updateWeight(0.5)} aria-label="Increase estimated weight"><Plus size={16} /></button></div><div className="weight-presets">{[["Small bag", 2], ["Medium bag", 5], ["Large bag", 10], ["Bulk", 25]].map(([label, value]) => <button className={weight === value ? "preset-active" : ""} key={label as string} onClick={() => updateWeight((value as number) - weight)}>{label}<b>{value as number} KG</b></button>)}</div></div><label className="ai-button"><Upload size={16} /><span><b>{wasteImage ? "Replace waste photo" : "Upload waste photo"}</b><small>PNG, JPEG, GIF, or WebP · max 8 MB · required for submission</small></span><input type="file" accept="image/png,image/jpeg,image/gif,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (file.size > 8 * 1024 * 1024) { toast.error("Choose an image smaller than 8 MB."); return; } onImageSelected(file); }} style={{ width: 120, height: 36, opacity: 0.01, position: "absolute", cursor: "pointer" }} /></label><button className="ai-button" onClick={runAi} disabled={!wasteImage || disabled}><ScanLine size={16} /><span><b>{disabled ? "Analyzing material..." : "Identify material"}</b><small>{wasteImage ? "Fallback classifier · vision service not configured" : "Select a photo first · estimate remains provisional"}</small></span><ArrowRight size={15} /></button>{aiDetected && classification && <div className="ai-result"><div><ScanLine size={18} /><span>MATERIAL ANALYSIS · {classification.isFallback ? "FALLBACK" : "AI"}</span></div><strong>{classification.detectedMaterial}</strong><p>{classification.confidence === null ? "No external vision model configured" : `Confidence ${Math.round(classification.confidence * 100)}%`} · Possible contamination: <b>{classification.contaminationEstimate}</b></p><small>{classification.notes}</small></div>}</div>}{step === 2 && <div className="schedule-body location-body"><label className="field-label">PICKUP LOCATION<input value={location} onChange={(event) => setLocation(event.target.value)} aria-label="Pickup location" /><small><MapPin size={13} /> Use current location</small></label><div className="map-preview"><div className="map-grid" /><span className="map-zone">COLLECTION ZONE 04</span><span className="map-pin"><MapPin size={18} /></span><span className="map-collector"><Truck size={14} /> NEARBY COLLECTOR</span><div className="map-route-line" /></div><div className="time-select"><span className="field-label">DATE & TIME WINDOW</span><div className="time-options">{["Today · 14:00 – 16:00", "Tomorrow · 10:00 – 12:00"].map((option) => <button className={slot === option ? "time-active" : ""} onClick={() => setSlot(option)} key={option}>{option.split(" · ")[0]}<span>{option.split(" · ")[1]}</span>{slot === option && <Check size={14} />}</button>)}</div></div><div className="network-note"><span><i /> DEMO NETWORK</span><b>3 collectors nearby</b><small>Estimated arrival: 18–32 min</small></div></div>}{step === 3 && <div className="schedule-body confirm-body"><div className="confirm-flow"><div className="confirm-icon"><GeneratorMark size={38} /></div><div className="confirm-line" /><div className="confirm-icon confirm-icon-end"><Truck size={19} /></div></div><div className="confirm-summary"><div><span>MATERIAL</span><b>{selected.join(" + ")}</b></div><div><span>ESTIMATED WEIGHT</span><b>{weight.toFixed(1)} KG</b></div><div><span>PICKUP</span><b>{slot}</b></div><div><span>LOCATION</span><b>{location}</b></div><div><span>ESTIMATED REWARD</span><b className="eco-text">~{Math.round(weight * 100)} ECO</b></div></div><div className="confirm-notice"><ShieldCheck size={16} /><span>Reward is finalized after verified weighing. The uploaded image is stored with this request.</span></div></div>}<footer className="schedule-footer"><button className="back-button" onClick={() => step === 1 ? onClose() : setStep(step - 1)} disabled={disabled}>{step === 1 ? "Cancel" : <><ArrowLeft size={15} /> Back</>}</button><button className="gen-primary" onClick={() => step < 3 ? setStep(step + 1) : onConfirm()} disabled={!canContinue || disabled}>{disabled ? "Saving pickup..." : step < 3 ? "Continue" : "Confirm pickup"} <ArrowRight size={15} /></button></footer></section></div>;
}

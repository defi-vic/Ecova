...
function Generator() {
  const { pickups, balance, transactions, impact, createPickup: createSharedPickup, redeemReward } = useEcovaData();
  ...
  const createPickup = () => {
    const id = createSharedPickup({ material: selectedMaterials.join(" + ") || "PET Plastic", estimatedWeight: weight, location, pickupWindow: slot });
...
function FlowDrawer({ pickup, onClose }: { pickup: Pickup; onClose: () => void }) {
  const completedIndex = pickup.status === "RECYCLED" ? 5 : pickup.status === "COLLECTED" ? 2 : pickup.status === "ARRIVED" || pickup.status === "ASSIGNED" ? 1 : 0;
  const verified = pickup.verifiedWeight ? `${pickup.verifiedWeight.toFixed(2)} KG` : "PENDING";
  return <div className="drawer-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="flow-drawer"><div className="drawer-header"><div><Kicker>MATERIAL FLOW</Kicker><h2>{pickup.id}</h2></div><button className="drawer-close" onClick={onClose}><X size={19} /></button></div><div className="drawer-summary"><div><span>MATERIAL</span><b>{pickup.material}</b></div><div><span>ESTIMATED</span><b>{pickup.estimatedWeight.toFixed(2)} KG</b></div><div><span>VERIFIED</span><b>{verified}</b></div><div><span>STATUS</span><strong>{pickup.status === "COLLECTED" ? "COLLECTED & WEIGHED" : pickup.status}</strong></div></div><div className="audit-note"><ShieldCheck size={16} /><span>CHAIN OF CUSTODY · WHO → WHAT → WHEN → WHERE → VERIFIED</span></div><div className="flow-audit">{flowSteps.map(([number, label, date, detail], index) => <div className={`audit-step ${index <= completedIndex ? "audit-step-done" : ""}`} key={number}><div className="audit-index">{index <= completedIndex ? <Check size={13} /> : number}</div>{index < flowSteps.length - 1 && <span className="audit-connector" />}<div className="audit-copy"><span>{label}</span><time>{index === 2 && pickup.verifiedAt ? pickup.verifiedAt : date}</time><p>{index === 2 && pickup.verifiedWeight ? `${pickup.verifiedWeight.toFixed(2)} KG VERIFIED` : detail}</p>{index === 2 && pickup.verifiedWeight && <b className="verified-weight"><Check size={12} /> WEIGHT VERIFIED</b>}{index === 5 && pickup.reward > 0 && <b className="eco-credit">+{pickup.reward} ECO</b>}</div></div>)}</div><div className="drawer-footer"><span>RECORD LAST UPDATED</span><b>{pickup.verifiedAt ?? "17 SEP 2026 · DEMO"}</b></div></aside></div>;
}

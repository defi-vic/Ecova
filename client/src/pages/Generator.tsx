...
type PickupStatus = "REQUESTED" | "ASSIGNED" | "COLLECTED" | "AT HUB" | "RECYCLED";

type Pickup = { id: string; date: string; material: string; weight: string; collector: string; status: PickupStatus; estimated?: string };

const seededPickups: Pickup[] = [
  { id: "EC-1048", date: "12 SEP", material: "PET Plastic", weight: "4.70 KG", collector: "Daniel O.", status: "RECYCLED" },
  { id: "EC-1042", date: "09 SEP", material: "Cardboard", weight: "8.20 KG", collector: "M. Okafor", status: "RECYCLED" },
  { id: "EC-1038", date: "04 SEP", material: "PET Plastic", weight: "3.10 KG", collector: "Daniel O.", status: "RECYCLED" },
];
...
function Generator() {
  ...
  const [pickups, setPickups] = useState(seededPickups);
  ...
  const runAi = () => { setAiDetected(false); window.setTimeout(() => setAiDetected(true), 450); };
  const createPickup = () => {
    const id = "EC-1051";
    setPickups((current) => [{ id, date: "TODAY", material: selectedMaterials.join(" + ") || "PET Plastic", weight: `${weight.toFixed(1)} KG EST.`, collector: "Matching in progress", status: "REQUESTED", estimated: `${weight.toFixed(1)} KG EST.` }, ...current]);
    setSelectedPickup(id); setModal(null); setScheduleStep(1); setView("pickups");
    window.setTimeout(() => {
      setPickups((current) => current.map((pickup) => pickup.id === id ? { ...pickup, status: "ASSIGNED", collector: "Matching complete" } : pickup));
      toast("Collector assigned", { description: `${id} has moved to the assigned stage in demo mode.` });
    }, 1200);
    toast("Pickup requested", { description: `${id} is now in your material record.` });
  };
  const redeem = () => { setBalance((current) => current - redeemAmount); setModal(null); toast("Demo redemption recorded", { description: `-${redeemAmount} ECO · Mobile Data · Prototype transaction` }); };
  ...
  return <div className="generator-app">
    <aside className={`generator-sidebar ${mobileNav ? "generator-sidebar-open" : ""}`}>
      ...
      <div className="sidebar-bottom"><div className="demo-label">DEMO ROLE</div><button className="demo-role-button" onClick={() => toast("Demo role: Generator", { description: "Role switching is a demonstration mechanism, not authentication." })}><span className="demo-avatar"><GeneratorMark size={17} /></span><span><b>Generator</b><small>Demo session</small></span><ChevronRight size={15} /></button><a className="back-to-site" href="/">← Back to public site</a></div>
    </aside>
...
function Overview({ pickups, onSchedule, onFlow, onView }: { pickups: Pickup[]; onSchedule: () => void; onFlow: (id?: string) => void; onView: (view: View) => void }) {
  return <>
    ...
    <section className="gen-metrics"><div className="metrics-head"><span>IMPACT SUMMARY</span><span className="demo-data"><i /> DEMO DATA</span></div><div className="metrics-row"><Metric label="TOTAL DIVERTED" value="342.0" unit="KG" detail="Material recovered" /><Metric label="CO₂e ESTIMATE" value="89.5" unit="KG" detail="Estimated impact" /><Metric label="VERIFIED PICKUPS" value="27" detail="Chain records" /><Metric label="ACTIVE STREAK" value="4" unit="WEEKS" detail="Keep it moving" /></div></section>
    <section className="active-pickup-card"><div className="active-pickup-top"><div><Kicker>ACTIVE PICKUP</Kicker><h2>EC-1048</h2></div><span className="status-status"><i /> COLLECTOR EN ROUTE</span></div><div className="active-pickup-grid"><div><span>MATERIAL</span><b>PET Plastic</b></div><div><span>WEIGHT</span><b>4.70 KG <small>VERIFIED</small></b></div><div><span>COLLECTOR</span><b>Daniel O.</b></div><div><span>EXPECTED TODAY</span><b>14:00 – 16:00</b></div><button onClick={() => onFlow("EC-1048")}>Track pickup <ArrowRight size={15} /></button></div><div className="pickup-route"><span className="route-node done"><Check size={11} /></span><span className="route-fill" /><span className="route-node active"><Truck size={11} /></span><span className="route-dash" /><span className="route-node"><Recycle size={11} /></span><div className="route-labels"><span>REQUESTED</span><span>EN ROUTE</span><span>RECYCLING HUB</span></div></div></section>
...
function Rewards({ balance, onRedeem }: { balance: number; onRedeem: () => void }) { return <section className="page-view"><div className="page-view-heading rewards-heading"><div><Kicker>GENERATOR / ECO WALLET</Kicker><h1>Value that follows<br /><em>the material.</em></h1></div><button className="gen-primary" onClick={onRedeem}><WalletCards size={16} /> Redeem ECO</button></div><div className="wallet-layout"><div className="wallet-balance"><div className="wallet-top"><span>ECO BALANCE</span><span className="demo-badge">DEMO VALUE</span></div><strong>{balance.toLocaleString()} <small>ECO</small></strong><p>Estimated value <b>₦{balance.toLocaleString()}</b></p><div className="wallet-bottom"><span>REWARD RATE</span><b>1 KG VERIFIED ≈ 100 ECO</b></div></div><div className="wallet-aside"><Sparkles size={20} /><p>Rewards are calculated from verified weight at collection, not AI estimates.</p></div></div><div className="transactions"><div className="activity-heading"><div><Kicker>TRANSACTION HISTORY</Kicker><h2>Movement in your wallet.</h2></div><span className="demo-data"><i /> SIMULATED</span></div><div className="transaction-list"><Transaction sign="+" amount="470" id="EC-1048" item="PET Plastic · Verified collection" date="13 SEP 2026" /><Transaction sign="+" amount="820" id="EC-1042" item="Cardboard · Verified collection" date="10 SEP 2026" /><Transaction sign="–" amount="500" id="REDEEM-004" item="Mobile Data · Prototype redemption" date="08 SEP 2026" negative /></div></div></section>; }
...
function Impact({ onFlow }: { onFlow: (id?: string) => void }) { return <section className="page-view"><div className="page-view-heading impact-page-heading"><div><Kicker>GENERATOR / IMPACT PASSPORT</Kicker><h1>Your waste has a<br /><em>measurable footprint.</em></h1><p>Every verified collection adds a line to your environmental record. Calculations shown are estimates for this demo.</p></div><button className="gen-secondary" onClick={() => onFlow("EC-1048")}>View verification history <ArrowRight size={15} /></button></div><div className="impact-stat-grid"><div><span>TOTAL MATERIAL RECOVERED</span><strong>342.0 <small>KG</small></strong></div><div><span>PLASTIC DIVERTED</span><strong>216.4 <small>KG</small></strong></div><div><span>VERIFIED COLLECTIONS</span><strong>27</strong></div><div><span>RECYCLING FACILITIES</span><strong>3</strong></div><div className="impact-stat-wide"><span>ESTIMATED CO₂e IMPACT</span><strong>89.5 <small>KG</small></strong><b>ESTIMATED · DEMO CALCULATION</b></div></div><div className="passport-record"><div className="passport-record-copy"><Kicker>IMPACT PASSPORT</Kicker><h2>A living record<br />of recovery.</h2><p>Generator ID <b>GC-DEMO-001</b></p><button className="text-button" onClick={() => onFlow("EC-1048")}>View verification history <ArrowRight size={15} /></button></div><div className="passport-record-card"><div className="passport-card-mark"><GeneratorMark size={40} /></div><span>ECOVA / ENVIRONMENTAL RECORD</span><strong>GC-DEMO-001</strong><div className="passport-card-grid"><div><small>MATERIALS RECOVERED</small><b>5 TYPES</b></div><div><small>VERIFIED COLLECTIONS</small><b>27 EVENTS</b></div><div><small>FACILITIES REACHED</small><b>3 HUBS</b></div><div><small>LATEST VERIFIED</small><b>EC-1048</b></div></div><div className="passport-card-footer"><ShieldCheck size={14} /> Record integrity maintained across material flow</div></div></div></section>; }
